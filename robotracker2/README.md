...
1. **Cargar canción** (`screens/load_song.py`): lista vertical de las canciones
   ...
   vacía, o no existen la novena pista ni la de luces (canciones LGPT de 8
   canales), se crean al cargar: Sample vacío, chain en la fila 0 del
   canal 0, de la pista extra y de la de luces. Quedan en memoria hasta
   Guardar.
...
3. **SONG** (`screens/song_view.py`): parrilla 256×10 de índices de chain
...
9. **PROJECT** (`screens/project_view.py`): menú reducido — **Tempo** y
   ...
   **Compact Instruments** elimina del banco los instrumentos que ninguna
   ...
   columna 0 de SONG, la novena pista o la de luces no existen (canciones
   LGPT de 8 canales), se crean en memoria — un Sample vacío y chains en
   la fila 0 del canal 0, de la pista extra y de la de luces — y quedan
   persistidos al guardar.
...
(más **luces**, fija): el icono y el nombre van juntos. Las filas van en el
mismo orden que las columnas de SONG (pista extra arriba, vocoder, robot y
luces abajo). Se ve en la
...
```json
{
  "tracks": ["drum", "bass", "synth", "synth", "noise", "synth", "vocoder", "robot", "synth", "lights"]
}
```

Sin la clave, esas mismas diez son el valor por defecto (vocoder, robot y
luces en los canales 7, 8 y 10, fijos; la extra es el canal 9). Las
canciones con la lista antigua de 9 siguen valiendo: la pista de luces
toma su valor fijo. Controles: **arr/abj**
